...
var TableCenter = /** @class */ (function () {
  function TableCenter(game, gamedatas) {
    ...
    document.getElementById('circuit').style.backgroundImage = "url('"
      .concat(g_gamethemeurl, 'img/Circuits/')
      .concat(gamedatas.circuit, ".jpg')");
...
var PlayerTable = /** @class */ (function () {
  ...
  PlayerTable.prototype.refreshHand = function (hand) {
    ...
    hand.forEach(function (card) {
      return (_this.game.cardsManager.getCardElement(card).dataset.playerColor = _this.game.getPlayer(_this.playerId).color);
    });
    return promise;
  };
  return PlayerTable;
...
var Heat = /** @class */ (function () {
  ...
  Heat.prototype.setup = function (gamedatas) {
    ...
    // TODO TEMP
    Object.values(gamedatas.constructors).forEach((constructor) => {
      $('circuit').insertAdjacentHTML('beforeend', `<div id='car-${constructor.id}'></div>`);
      let cell = window['USA_DATAS'][constructor.carCell];
      let scale = 1650 / 1280;
      dojo.style(`car-${constructor.id}`, {
        position: 'absolute',
        left: scale * cell.x + 'px',
        top: scale * cell.y + 'px',
        width: '61.08px',
        height: '34px',
        background: 'url(' + g_gamethemeurl + '/img/cars.png)',
  ...
  Heat.prototype.onHandCardSelectionChange = function (selection) {
    if (this.gamedatas.gamestate.name == 'planification') {
      ...
      var label = allowed
        ? _('Set gear to ${gear} an play selected cards').replace('${gear}', ''.concat(selection.length)) +
          (Math.abs(selection.length - gear) == 2 ? ' (+1 [Heat])' : '')
