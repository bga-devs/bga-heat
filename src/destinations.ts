class DestinationsManager extends CardManager<TechnologyCard> {
    constructor (public game: HeatGame) {
        super(game, {
            getId: (card) => `destination-${card.id}`,
            setupDiv: (card: TechnologyCard, div: HTMLElement) => {
                div.classList.add('heat-destination');
                div.dataset.cardId = ''+card.id;
                div.dataset.type = ''+card.type;
            },
            setupFrontDiv: (card: TechnologyCard, div: HTMLElement) => { 
    ...
    private getTooltip(destination: TechnologyCard): string {
        let message = `
